...
with open(os.path.join(os.path.dirname(__file__), "openllm-config.yaml")) as f:
...
SERVICE_CONFIG["envs"].extend([
    {"name": "VLLM_USE_V1", "value": "1" if USE_V1 else "0"},
...
@bentoml.service(
    **SERVICE_CONFIG, image=IMAGE, labels=dict(generator="openllm", owner="bentoml-team", aliases=",".join(ALIASES))
)
class VLLM:
